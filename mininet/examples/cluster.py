...
class RemoteLink( Link ):
    "A RemoteLink is a link between nodes which may be on different servers (SSH Link)"
    ...
    def makeIntfPair( self, intfname1, intfname2, addr1=None, addr2=None,
        ...
        server1 = getattr( node1, 'server', 'localhost' ) #Obtiene el valor de server y localhost del objeto node1
    ...
    def makeTunnel( self, node1, node2, intfname1, intfname2,
        ...
        # -n: close stdin
	# ssh -w: crea un tunel entre local:dest. Those interfaces may be numerical ID or "any"
        ...
        for node, intf, addr in ( ( node1, intfname1, addr1 ),
            ...
            if not addr:
                node.cmd( 'sudo ip link set tap9 down' ) #personalizada
                result = node.cmd( 'sudo ip link set tap9 name', intf ) #he hecho un cambio agregando sudo
                node.cmd( 'sudo ip link set', intf, 'up' ) #personalizada
            else:
                node.cmd( 'sudo ip link set tap9 down' ) #personalizada
                result = node.cmd( 'sudo ip link set tap9 name', intf,
                                    'address', addr )
                node.cmd(  'sudo ip link set', intf, 'up') #personalizada
            if result:
...
class MininetCluster( Mininet ):
    ...
    def __init__( self, *args, **kwargs ):
        ...
        if params.pop( 'precheck' ):
            self.precheck()
        self.connections = {}
        self.placement = params.pop( 'placement', SwitchBinPlacer ) #Usa SwitchBinPlacer by default
    ...
    def placeNodes( self ): #Prestarle atenciona  este metodo...
