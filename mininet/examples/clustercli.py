...
"CLI pepe for Mininet Cluster Edition prototype demo"
...
class ClusterCLI( CLI ):
	...
	output('entre a la funcion\n')
        if not nx:
            try:
                # pylint: disable=import-error,no-member
		output('entre al try\n')
                import networkx
		output('se importo networkx\n')
                nx = networkx  # satisfy pylint
                from matplotlib import pyplot
		output('se importo matplot y pyplot\n')
                plt = pyplot   # satisfy pylint
                import pygraphviz
		output('se importo pygraphviz\n')
