...
var ws = new WebSocket("ws://" + location.host + "/v1.0/topology/ws"); //Crea un websocket
    ...
    d3.json("/stats/flow/" + dpid, function(e, data) {
      ...
      /*  li = elem.console.append("ul")
        ...
        li.data(flows).enter().append("li")
            .text(function (d) { return JSON.stringify(d, null, " "); });*/ /*Esto es para poner el string de los flows al final de la pagina*/
    });
...
function is_valid_link(link) {
    return (link.src.dpid < link.dst.dpid) //retorna true or false
}

var topo = { //Objeto Topologia
    nodes: [], //arreglo de nodos
    links: [], //arreglo de links

    node_index: {}, // dpid -> index of nodes array
    initialize: function (data) {
        this.add_nodes(data.switches);
        this.add_links(data.links);
    }, //El valor initialize es una funcion que recive una data y ejecuta las dos funciones de dentro
    add_nodes: function (nodes) {
        for (var i = 0; i < nodes.length; i++) {
            this.nodes.push(nodes[i]); //Agrega al arreglo de nodos los que recibe por parametro
        }
